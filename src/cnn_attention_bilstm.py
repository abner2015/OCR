...
class CNNATTBiLSTM(gluon.HybridBlock):
    '''
    recognise the image
    ...
    def __init__(self,num_downsamples=2,resnet_layer_id = 4,rnn_hidden_states=200,rnn_layers=1,max_seq_len=100,ctx=mx.gpu(0),**kwargs):
        super(CNNATTBiLSTM,self).__init__(**kwargs)
        self.p_dropout = 0.5
        ...
        with self.name_scope():
            ...
            with self.encoders.name_scope():
                for i in range(self.num_downsamples):
                    if i == 0:
                        input_size = 2048  # input 128x64
                        #input_size = 3584 #input 224
                    else:
                        input_size = 512  # input 128x64
    ...
    def get_attention(self):
        ...
        attention = gluon.nn.HybridSequential(prefix="attention")
        alignment_dim = 2048
        hidden_dim = 2000
        ...
        with attention.name_scope():
            attention.add(gluon.nn.Dense(
                ...
                activation="tanh", flatten=True,use_bias=False))
            attention.add(gluon.nn.Dense(alignment_dim, in_units=alignment_dim,
                                        flatten=True,use_bias=False))
    ...
    def get_body(self,resnet_layer_id):
        ...
        pretrained_2 = resnet34_v1(pretrained=True,ctx=mx.gpu(0))
    ...
    def hybrid_forward(self, F, x):
        ...
        att = F.softmax(att,axis=0)
        #encoder_output = hs.swapaxes(0,1)
        decoder_context = F.dot(att,hs)
